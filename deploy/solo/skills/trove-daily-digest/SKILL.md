...
`nugget_tasks(horizon="week")` — tasks due in the next 7 days (excluding
today, which is bucket 1). Group by day if there are multiple, or list
them plainly.
...
These are open-ended items that haven't been scheduled yet. Show a count
and a couple of examples, not a full dump.
...
**Conscious format decision:** The SOUL says "no bullet reports unless she
asks." A daily digest is inherently a list, but the cron setup IS her
